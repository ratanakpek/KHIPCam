 ...
 */
public class MyVideoView extends FrameLayout implements IUserMethods, TextureView.SurfaceTextureListener,
    ...
    private Uri mSource;
    private MyVideoViewCallback mCallback;
    ...
    private void init(Context context, AttributeSet attrs) {
        ...
        if (attrs != null) {
            TypedArray a = context.getTheme().obtainStyledAttributes(
                    attrs,
                    R.styleable.MyVideoView,
                    0, 0);
            try {
                String source = a.getString(R.styleable.MyVideoView_cv_source);
                if (source != null && !source.trim().isEmpty())
                    mSource = Uri.parse(source);

                //noinspection WrongConstant
                mLeftAction = a.getInteger(R.styleable.MyVideoView_cv_leftAction, LEFT_ACTION_RESTART);
                //noinspection WrongConstant
                mRightAction = a.getInteger(R.styleable.MyVideoView_cv_rightAction, RIGHT_ACTION_NONE);

                mCustomLabelText = a.getText(R.styleable.MyVideoView_cv_customLabelText);
                mRetryText = a.getText(R.styleable.MyVideoView_cv_retryText);
                mSubmitText = a.getText(R.styleable.MyVideoView_cv_submitText);
                mBottomLabelText = a.getText(R.styleable.MyVideoView_cv_bottomText);


                int restartDrawableResId = a.getResourceId(R.styleable.MyVideoView_cv_restartDrawable, -1);
                int playDrawableResId = a.getResourceId(R.styleable.MyVideoView_cv_playDrawable, -1);
                int pauseDrawableResId = a.getResourceId(R.styleable.MyVideoView_cv_pauseDrawable, -1);
                ...
                mHideControlsOnPlay = a.getBoolean(R.styleable.MyVideoView_cv_hideControlsOnPlay, true);
                mAutoPlay = a.getBoolean(R.styleable.MyVideoView_cv_autoPlay, false);
                mControlsDisabled = a.getBoolean(R.styleable.MyVideoView_cv_disableControls, false);

                mThemeColor = a.getColor(R.styleable.MyVideoView_cv_themeColor,
                        Util.resolveColor(context, R.attr.colorPrimary));

                mAutoFullscreen = a.getBoolean(R.styleable.MyVideoView_cv_autoFullscreen, false);
                mLoop = a.getBoolean(R.styleable.MyVideoView_cv_loop, false);
            } finally {
                a.recycle();
            }
        } else {
        ...
        if (mRetryText == null)
            mRetryText = context.getResources().getText(R.string.cv_retry);
        if (mSubmitText == null)
            mSubmitText = context.getResources().getText(R.string.cv_submit);

        if (mRestartDrawable == null)
            mRestartDrawable = AppCompatResources.getDrawable(context, R.drawable.cv_action_restart);
        if (mPlayDrawable == null)
            mPlayDrawable = AppCompatResources.getDrawable(context, R.drawable.cv_action_play);
        if (mPauseDrawable == null)
            mPauseDrawable = AppCompatResources.getDrawable(context, R.drawable.cv_action_pause);
    }
    ...
    @Override
    public void setCallback(@NonNull MyVideoViewCallback callback) {
        mCallback = callback;
    }

    @Override
    public void setProgressCallback(@NonNull VideoProgressCallback callback) {
    ...
    protected void onFinishInflate() {
        ...
        mProgressFrame = li.inflate(R.layout.cv_include_progress, this, false);
        ...
        // Inflate controls
        mControlsFrame = li.inflate(R.layout.cv_include_controls, this, false);
        ...
        final MyVideoView myVideoView = this;
        ...
        if (mControlsDisabled) {
            ...
            mClickFrame.setOnClickListener(new OnClickListener() {
                ...
                public void onClick(View view) {
                    toggleControls();
                    mCallback.onClickVideoFrame(myVideoView);
                }
            });
        }
